...
for p1, p2 in edges[:1000]:
    merge_sets(p1, p2)

print(reduce(lambda x, y: x * y, [value for _, value in sorted(child_count.items(), key=lambda item: item[1], reverse=True)[:3]]))
print(sum(child_count.values()))
